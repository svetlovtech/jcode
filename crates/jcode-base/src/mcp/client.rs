...
/// Which remote wire protocol a `RemoteTransport` speaks.
...
impl McpClient {
    ...
    pub async fn connect_in_dir(
        ...
        if config.is_remote() {
            return Self::connect_remote(name, config).await;
        }
    ...
    async fn connect_remote(name: String, config: &McpServerConfig) -> Result<Self> {
        let url = config
            .url
            .clone()
        ...
        let session_id: Arc<std::sync::RwLock<Option<String>>> =
            Arc::new(std::sync::RwLock::new(None));
        let endpoint: Arc<std::sync::RwLock<Option<String>>> =
            Arc::new(std::sync::RwLock::new(None));
        ...
        let mut client = Self {
            handle,
            transport: ClientTransport::Remote(RemoteTransport {
                kind,
                url,
                closed,
                session_id,
    ...
    pub fn is_running(&mut self) -> bool {
        match &mut self.transport {
            ...
            ClientTransport::Remote(remote) => !remote.closed.load(Ordering::SeqCst),
        }
    }
    ...
    pub async fn shutdown(&mut self) {
        ...
        match &mut self.transport {
            ...
            ClientTransport::Remote(remote) => {
                // Signal background tasks to stop (the SSE stream task checks
                // this between reads; the driver stops accepting messages).
                remote.closed.store(true, Ordering::SeqCst);
                // Best-effort session teardown for streamable HTTP.
                if remote.kind == RemoteKind::StreamableHttp {
                    let session = remote.session_id.read().unwrap().clone();
                    let mut request = reqwest::Client::new().delete(&remote.url);
                    if let Some(session) = &session {
                        request = request.header("Mcp-Session-Id", session);
                    }
                    if let Ok(response) =
                        tokio::time::timeout(std::time::Duration::from_secs(2), request.send())
                            .await
                    {
