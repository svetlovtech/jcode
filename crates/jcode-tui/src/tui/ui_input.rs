...
pub(super) fn draw_overscroll_status(frame: &mut Frame, app: &dyn TuiState, area: Rect) {
    ...
    if crate::config::config().display.footer_style_advanced() {
        ...
        let _ = secs;
        // Fall through by rendering directly: the advanced style draws its
        ...
        let spans_width = |sp: &[Span<'static>]| {
            sp.iter().map(|x| x.content.chars().count()).sum::<usize>()
        };
        let mut chosen = full_countdown.clone();
        if spans_width(&spans) + full_countdown.len() + 1 > total_width {
            chosen = compact_countdown.clone();
        }
        if spans_width(&spans) + chosen.len() + 1 > total_width {
            let truncated = overscroll_truncate_spans(
                spans,
