...
impl App {
    pub(super) fn command_help(&self, topic: &str) -> Option<String> {
        ...
        let help = match topic.as_str() {
            ...
            "export" => {
                "/export\nExport this session to a self-contained HTML viewer (sidebar index, search, collapsible tool outputs) and print the file path.\n\n/export json\nExport the raw session JSON (session + viewer payload) instead of HTML.\n\n/export json /tmp/copy.json\nWrite to an explicit path. The HTML file embeds its own data, opens offline in any browser, and its JSON button re-downloads the payload.\n\n/export swarm\nInclude related subagent sessions (parent/child links, same working dir within ±6h) and render a per-session Gantt timeline."
            }
            "cache" => {
