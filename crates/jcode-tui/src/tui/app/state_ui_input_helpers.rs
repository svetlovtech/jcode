...
const REGISTERED_COMMANDS: &[RegisteredCommand] = &[
    ...
    RegisteredCommand::public(
        "/export",
        "Export session to self-contained HTML (or --format json via /export json)",
    ),
...
impl App {
    ...
    fn command_candidates(&self) -> Vec<(String, &'static str)> {
        let fingerprint = super::quick_prompts::prompt_sources_fingerprint();
        ...
        for (name, text) in crate::config::config().prompts.valid_entries() {
            ...
            if seen.insert(command.clone()) {
                ...
                commands.push((command, super::quick_prompts::intern_hint(&hint)));
            }
        }
