...
/// Fork: `jcode session export <id|path> [--format html|json] [-o path] [--open]`.
///
...
pub fn run_session_export_command(
    ...
    let (session, source_file) = if source_path.exists() {
        (
            session::Session::load_from_path(source_path)?,
            Some(source_path.to_path_buf()),
        )
    } else {
        let resolved_id = session::find_session_by_name_or_id(session_ref)?;
        let path = session::session_path(&resolved_id)?;
        (session::Session::load(&resolved_id)?, Some(path))
    };
    ...
    let input = jcode_export_core::SessionExportInput {
        ...
        created_at: Some(session.created_at.to_rfc3339_opts(chrono::SecondsFormat::Millis, true)),
        updated_at: Some(session.updated_at.to_rfc3339_opts(chrono::SecondsFormat::Millis, true)),
